...
<fmt:message key="text.delete.body.text" var="delete_body_text"/>
<fmt:message key="button.yes" var="yes"/>
<fmt:message key="button.no" var="no"/>
<fmt:message key="page.audi.dealer.delete.body.page" var="delete_page"/>
...
<div align="center">
    <h3><u>${delete_body_text} ${param.id}?</u></h3>
    <form action=<%=request.getContextPath()%>"delete" method="post">
        <input type="hidden" name="id" value="${param.id}">
        <a class="btn btn-outline-success btn-sm" href=<%=request.getContextPath()%>"/body/find-all"
           role="button">${no}</a>
